...
#
#table DP[101][n+1], for a book_list of length n.
#
#DP[i][j] denotes the solution for a box of capacity i, and first j books from the given list of books.
...
def populate_table(scaled_cap, scaled_weights):
    ...
    If the box is filled to capacity with the first j books, we don't calculate further.
    Return last book index, and table to achieve max possible total weight with given list of books.
    Worst Case Complexity: O(N*W). N is the number of books, W is the capacity of box.
    """
...
def collect_to(box_id, books, scaled_weights):
    ...
    Worst case complexity: O(N+W). N is number of books, W is the capacity of box.
    """
