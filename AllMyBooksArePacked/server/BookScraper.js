...
var BookScraper = (function BookScraper () {
  ...
  function getPrice($) {
    ...
    //selection came with a lot of whitespace - this cleans it up
    ...
    $('table', '#detail-bullets').find('li').each(function(i, elem) {
      ...
      if(!doneSearchingList) {
        ...
        if(listItem.includes('ISBN-10')) {
          ...
          //original string 'ISBN-10: 140006922X'
          isbn = listItem.substr(-10);

        } else if(listItem.includes('Shipping Weight')) {

          //original string 'Shipping Weight: 1.2 pounds (View shipping rates and policies)'
          shipping_weight = listItem.substr(17, 10);
